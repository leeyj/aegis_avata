# AEGIS Plugin-X: 확장 모듈 개발 가이드 (v2.9.0)
...
**`csp_domains` 실전 JSON 예시:**
```json
"csp_domains": {
    "connect-src": ["https://api.github.com", "https://*.openapi.com"],
    "img-src": ["https://*.openweathermap.org", "https://cdn.example.com"],
...
| **v2.4.5** | 2026-03-04 | **AXC (AEGIS Extreme Cache)** 및 **병렬 하이드레이션** 도입. 0ms 광속 부팅 및 DOM 순서 고정 아키텍처 수립 |

---

**AEGIS Plugin-X Standard v2.9.0 Documentation**
**이 문서는 AI 및 개발자 모두를 위한 구속력 있는 가이드입니다.**
**핵심 원칙: 사용자에게 JSON 편집을 강요하지 않는다.**

> 💡 **빠른 시작**: `python create_plugin.py --id my-widget --name "나의 위젯"` 으로 모든 규칙이 자동 준수된 템플릿을 생성할 수 있습니다. 자세한 사항은 `create_plugin.py --help`를 참조하세요.
---
...
4.  백엔드 서비스(`service.py`)에서는 `utils.strip_markdown_wrappers()`를 사용하여 AI의 응답 래퍼(```json 등)를 반드시 제거한 후 파싱하십시오.
...
### 9-2. 인텐트 라우팅 가이드
터미널 입력 첫 문자를 통해 시스템은 다음 액션을 즉각 판단합니다.
* **`#` (확정적 웹 검색)**: 예시 `#오늘 서울 날씨`. 시스템 AI를 바이패스하고 구글 검색 도구를 강제.
* **`@` (컨텍스트 다중 알리아스 지원)**: 예시 `@스케줄러`. 해당 플러그인의 상태(Context) 데이터를 실시간으로 읽어 AI 대답에 주입합니다. **`register_context_provider`** 에 선언된 aliases 가 동적으로 등록됩니다.
* **`/` (직접 명령 처리)**: 특정 플러그인 동작 수행.
* *(없음)*: 자유로운 자율 판단 (Gemini/Ollama).
