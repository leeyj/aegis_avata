# AEGIS Plugin-X 프레임워크 레퍼런스 가이드 (FRAMEWORK REFERENCE) v3.7.0

이 문서는 AEGIS Plugin-X (v3.7.0) 아키텍처 환경에서 개발자 및 AI 에이전트가 활용할 수 있는 모든 레퍼런스를 총망라합니다. v3.4.0에서는 **글로벌 다국어(I18n) 통합 지침**과 **플랫폼 독립적 메시징 허브(BotManager)**가 도입되었으며, v3.7.0에서는 **확정적 제어(Deterministic Control)**와 **BotManager 중앙 라우팅**이 추가되어 시스템의 신뢰도와 반응 속도가 극대화되었습니다.
