    ...
    async _loadExports() {
        ...
        if (!this._cachedExports) {
            try {
                ...
                this._cachedExports = { sensors: [], commands: [], actions: [] };
            }
        }
        ...
        // [v2.0] 액션 드롭다운 채우기 (동적 플러그인 액션 추가)
        const actionSelect = document.getElementById('routine-action');
        if (actionSelect) {
            const existingDynamic = actionSelect.querySelectorAll('.dynamic-action');
            existingDynamic.forEach(el => el.remove());

            const actions = this._cachedExports.actions || [];
            if (actions.length > 0) {
    ...
    toggleActionFields() {
        ...
        if (descEl) {
            if (action.startsWith('plugin:')) {
                const opt = document.querySelector(`#routine-action option[value="${action}"]`);
