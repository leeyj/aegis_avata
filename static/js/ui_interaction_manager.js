...
// [v4.1] UI Interaction variables moved to window for cross-iframe bridging
window.lastMouseX = 0;
window.lastMouseY = 0;
let clickCount = 0;
let clickTimer = null;
...
function initInteractions() {
    ...
    // 1. 포인터 다운 (이동 시작, 우클릭 탐지 - 롱클릭 리셋 기능 삭제됨)
    ...
    window.addEventListener('pointermove', (e) => {
        ...
        if (typeof window.lastMouseX !== 'undefined') {
            window.offsetX += (e.clientX - window.lastMouseX);
            window.offsetY += (e.clientY - window.lastMouseY);
        }
    ...
    window.addEventListener('wheel', (e) => {
        ...
        const shouldSkip = path.some(el =>
            el.classList && (
                ...
                el.classList.contains('glass-panel-v4') ||
                el.classList.contains('fixed-plugin-wrapper')
            )
        );
...
function resetAvatarLayout() {
    ...
    if (window.messageBroker) {
        window.messageBroker.broadcast('SPEAK', { text: "레이아웃을 초기화할게요.", visualType: 'system' });
    }
