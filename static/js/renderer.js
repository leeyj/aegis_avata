...
function initPixiApp() {
    ...
    console.log("[Renderer] Initializing PixiJS App on canvas:", canvas);

    if (!canvas) {
        console.error("[Renderer] CRITICAL: live2d-canvas not found in DOM!");
        return;
    }

    if (typeof PIXI === 'undefined') {
        console.error("[Renderer] CRITICAL: PIXI is not defined!");
        return;
    }
    ...
    try {
        ...
        console.log("[Renderer] PixiJS Application initialized successfully.");
    } catch (e) {
        console.error("[Renderer] PixiJS Initialization failed:", e);
    }
