...
function initSinglePanelDragging(panel) {
    ...
    panel.onpointerdown = (e) => {
        ...
        const isInteractive = path.some(el =>
            ...
            el.closest('button, select, input, textarea, label, a, .no-drag, .resize-handle, .brief-speaker, .interactive, .clickable, svg, path, i')
        );
...
function applyUIPositions() {
    ...
    for (const [id, pos] of Object.entries(window.uiPositions)) {
        ...
        if (el) {
            // [v2.4.2] Fixed plugins should NEVER be repositioned by drag manager ratio logic
