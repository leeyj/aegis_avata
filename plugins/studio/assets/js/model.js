...
async function refreshModelList() {
    console.log("[Studio:Model] Refreshing model list...");
    try {
        const url = 'models';
        console.log(`[Studio:Model] Fetching from: ${url}`);
        const res = await window.context.fetch(url);
        console.log(`[Studio:Model] Fetch response status: ${res.ok ? 'OK' : 'FAIL'}`);
        
        const models = await res.json();
        console.log("[Studio:Model] Models received:", models);
        
        const select = document.getElementById('model-select');

        if (!models || models.length === 0) {
            console.warn("[Studio:Model] No models returned from server.");
...
async function loadModel(name) {
    ...
    try {
        ...
        const res = await window.context.fetch(`model_info/${name}`);
        ...
        // 관리 버튼 (저장 버튼은 항상 활성화하여 맛보기를 허용합니다)
        const saveBtn = document.getElementById('save-alias-btn');
        if (saveBtn) {
            saveBtn.disabled = false;
            saveBtn.innerText = "Save alias.json";
        }

        const modelBase = `models/${name}/`;
        const modelSettingsRes = await window.context.fetch(`${modelBase}${data.model_settings_file}`);
        const modelJson = await modelSettingsRes.json();
        
        // [v4.2.2] IMPORTANT: DO NOT use context.resolve() here. 
        // Studio models are served by the specialized router.py API, not the static asset server.
        modelJson.url = `/api/plugins/${window.context.manifest.id}/${modelBase}${data.model_settings_file}`;

        // Inject/Sync Asset References from Studio Discovery
        if (window.modelAssets.expressions && window.modelAssets.expressions.length > 0) {
            ...
            modelJson.FileReferences.Expressions = window.modelAssets.expressions.map(e => ({
                ...
                File: e // Studio already provides relative paths like expressions/foo.json
            }));
        }
...
async function applyModelToAegis() {
    ...
    try {
        ...
        const res = await window.context.fetch(`apply_model/${name}`, { method: 'POST' });
...
async function fixModel() {
    ...
    try {
        ...
        const res = await window.context.fetch(`fix_model/${name}`, { method: 'POST' });
        const result = await res.json();
