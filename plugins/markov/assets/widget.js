...
export default {
    init: async function (shadow, context) {
        console.log("%c[Markov Plugin] Initializing with shadow and context...", "color: #ff00ff; font-weight: bold;");
        console.log("[Markov Plugin] Context:", context);
        console.log("[Markov Plugin] ShadowRoot:", shadow);
        ...
        // [v1.0.5] Canvas 직접 바인딩 대신 window 레벨에서 캡처 후 타겟 체크 (Z-Index 대응)
        window.addEventListener('mousedown', (e) => {
            // 클릭된 요소가 라이브2D 캔버스인지 확인
            const isModelClicked = e.target && (e.target.id === 'live2d-canvas' || e.target.closest('#live2d-canvas'));

            if (isModelClicked) {
                const now = Date.now();
                if (now - lastPokeTime < 600) {
                    pokeCount++;
                    console.log(`%c[Markov] Poke Chain! count: ${pokeCount}`, "color: #ff9f43; background: #222; padding: 2px 4px;");
                } else {
                    pokeCount = 1;
                    console.log("[Markov] Starting new poke sequence.");
                }
                lastPokeTime = now;

                // 3번 이상 연속 클릭하면 화내거나 아무말 시작
                if (pokeCount >= 3) {
                    console.log("%c[Markov] TRIPLE POKE TRIGGERED! - Generating nonsense...", "background: #eb3b5a; color: #fff; font-weight: bold; padding: 5px;");
                    const nonsense = generateMarkovText();

                    // Live2D 모션/표정 랜덤 트리거
                    if (window.ModelController && window.ModelController.model) {
                        try {
                            const motions = ['TapBody', 'TapHead', 'Idle'];
                            const randomMotion = motions[Math.floor(Math.random() * motions.length)];
                            window.ModelController.model.motion(randomMotion);
                        } catch (err) { console.error("[Markov Plugin] Motion Error:", err); }
                    }

                    // TTS 재생 및 말풍선 표시
                    if (typeof window.speakAndBubble === 'function') {
                        window.speakAndBubble(nonsense, {
                            duration: 4000,
                            mood: "neutral"
                        });
