...
export default {
    ...
    init: async function (root, context) {
        ...
        // 2. DOM 요소 참조
        const statusEl = root.getElementById('weather-status');
        const tempEl = root.getElementById('weather-temp');
        const iconEl = root.getElementById('weather-icon');
        ...
        const updateWeather = async () => {
            try {
                ...
                if (statusEl && data.status) {
                    // 시스템 번역 단축 함수 사용
                    const template = context._t('widgets.weather_template') || "{city}: {status}";
        ...
        context.onSystemEvent('WEATHER_SYNC', () => {
            updateWeather();
        });
        ...
        // [v3.0] 터미널 명령어 등록
        context.registerCommand('/weather', () => {
            updateWeather();
            context.appendLog('WEATHER', '🌤️ 날씨 정보를 업데이트합니다.');
        });
    },
