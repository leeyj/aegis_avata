...
export default {
    ...
    init: async function (root, context) {
        ...
        const listEl = root.getElementById('calendar-list');
        if (!listEl) return;
        ...
        const updateCalendar = async () => {
            try {
                const res = await context.fetch('events');
        ...
        context.onSystemEvent('CALENDAR_SYNC', () => updateCalendar());
        context.onSystemEvent('CALENDAR_LIST_SYNC', () => updateCalendar());
        ...
        // 3. 터미널 명령어 등록
        context.registerCommand('/calendar', () => {
            updateCalendar();
            context.appendLog('CALENDAR', '📅 일정을 새로고침합니다.');
        });
    },
