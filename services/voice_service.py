...
def generate_cached_tts(text, prefix="tts", voice=None):
    ...
    if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
        ...
        print(f"[VoiceService] Generating new TTS for text: {text[:20]}...")
        success = generate_edge_tts(text, current_voice, output_path, settings)
        if not success:
            print("[VoiceService] ❌ Failed to generate TTS file.")
            return None
        print(f"[VoiceService] ✅ TTS file generated: {filename}")
    ...
    def _run_async_in_thread():
        ...
        async def _generate():
            try:
                ...
                print(f"[VoiceService] ❌ Internal Edge TTS Error: {e}")
                return False
