...
# AEGIS Intelligence Dashboard (v4.0 "Plugin-X")

> [!IMPORTANT]
> **v4.0 "Plugin-X" Ecosystem 정식 릴리즈 안내**
> - 본 `main` 브랜치는 새로운 플러그인 아키텍처가 적용된 v4.0 버전입니다.
> - 기존 v3.8(Legacy) 코드를 계속 사용하시려는 개발자나 사용자는 [**v3.8-legacy**](https://github.com/leeyj/aegis_avata/tree/v3.8-legacy) 브랜치를 이용해 주세요.
...
[![Docs](https://img.shields.io/badge/Documentation-Comprehensive-blue?style=for-the-badge&logo=readthedocs)](docs/README.md)
[![AI Ready](https://img.shields.io/badge/AI_Plugin_Dev-Ready-brightgreen?style=for-the-badge&logo=openai)](docs/for_developer/QUICK_START.md)
...
> | 대상 | 문서 | 설명 |
> |---|---|---|
> | � **일반 사용자** | 📖 [USER GUIDE](docs/manual/USER_GUIDE.md) | 설치부터 설정까지 종합 가이드 |
> | � **개발자** | 🚀 [QUICK START](docs/for_developer/QUICK_START.md) | 5분 안에 첫 플러그인 만들기 |
> | � **개발자** | 📐 [PLUGIN SPEC](docs/for_developer/PLUGIN_SPEC.md) | 모든 규칙·API·스키마 레퍼런스 |
> | 🤖 **AI 에이전트** | 🤖 [AI PROMPT](docs/for_developer/AI_AGENT_PROMPT.md) | ChatGPT/Claude에 복사-붙여넣기 → 즐시 위젯 개발 |
> | 🏗️ **아키텍처** | 🏗️ [ARCHITECTURE](docs/ARCHITECTURE.md) | 시스템 설계 철학 및 전체 구조 |
...
```bash
# 1번: 골격 자동 생성 (약 20개 규칙 자동 준수)
python create_plugin.py --id my-widget --name "나의 위젯"

# 2번: AI에게 맡기기 — AI_AGENT_PROMPT.md를 ChatGPT/Claude에 복사-붙여넣기
# "비트코인 실시간 가격 위젯 만들어줘" → 즉시 개발 가능
```

**왜 가능한가요?**
- 📋 **구조화된 문서체계**: 모든 규칙·API·스키마가 [PLUGIN_SPEC.md](docs/for_developer/PLUGIN_SPEC.md) 한 문서에 정의 (Single Source of Truth)
- 🤖 **AI 전용 프롬프트**: [AI_AGENT_PROMPT.md](docs/for_developer/AI_AGENT_PROMPT.md)를 복사하면 어떤 AI든 즉시 플러그인 개발 가능
- 🛠️ **보일러플레이트 생성기**: `create_plugin.py`가 폴더 구조, 매니페스트, 라우터, Context API 연동 등 ~20개 규칙을 자동 적용
- 🔌 **플러그 앨 플레이**: 폴더 추가/삭제만으로 기능 확장/제거 — 코어 파일 수정 불필요
...
- **다국어 지원(Global I18n) ✨**: 한국어와 영어는 물론 사용자의 환경 설정에 따른 자동 언어 전환 및 AI 페르소나 현지화 지원.
- **통합 메시징(Discord Bot) ✨**: 대시보드 외부에서도 디스코드를 통해 내 일정을 확인하고, 알람을 설정하며, AI와 대화 가능.
...
AEGIS v2.0부터 터미널 방식의 오버레이 HUD가 도입되어 다양한 명령을 직관적으로 내릴 수 있습니다. 대시보드 어디서든 단축키 **`Shift + ~`** (Quake 방식)를 누르면 터미널 HUD가 나타납니다.

![터미널 HUD 스크린샷 1 (이 부분에 첫 번째 스크린샷이 들어갈 예정입니다)](img/user_guide_terminal1.png)
*(단축키 호출 및 명령어 입력 화면)*

### 🖥️ AEGIS 통합 명령어 체계 (Unified Command routing v3.4)

v3.4.0부터 모든 입력 채널(Web Terminal, Discord)이 동일한 명령어 체계를 따릅니다. 명령어 첫 글자에 따라 AI의 동작 모드가 결정됩니다.

**1. 🧠 하이브리드 추론 ( `/@` )**
- 로컬 위젯 데이터(Context)와 실시간 외부 웹 검색을 결합하여 가장 정확하고 지능적인 답변을 생성합니다.
- 예: `/@오늘 내 일정과 어울리는 점심 메뉴 추천해줘`

**2. 🏠 로컬 전용 요약 ( `/` )**
- 외부 검색 없이 **오직 내 시스템의 데이터**만을 사용하여 답합니다. 보안이 필요한 정보나 정시 보고에 적합합니다. (기존 별칭 명령 포함)
- 예: `/뉴스`, `/일정`, `/주식 브리핑해줘`

**3. 🌐 확정적 웹 검색 ( `/#` )**
- 내 시스템 컨텍스트를 제외하고, 순수하게 외부 검색 엔진만을 활용하여 최신 정보를 가져옵니다.
- 예: `/#현재 엔비디아 주가 알려줘`, `/#오늘 서울 주요 뉴스`

**4. 🤖 자율 대화 (접두사 없음)**
- 접두사가 없으면 AI가 내 컨텍스트를 사용할지 말지 자율적으로 판단하여 대화합니다.

**현재 등록된 인지 가능 별명 목록:**
- **scheduler**: `scheduler`, `루틴`, `스케줄러`, `예약`
- **calendar**: `calendar`, `일정`, `달력`, `스케줄`
- **stock**: `stock`, `주식`, `증시`
- **emails**: `emails`, `메일`, `이메일`, `편지`, `gmail`
- **system-stats**: `system-stats`, `시스템`, `상태`, `컴퓨터`, `system`
- **finance**: `finance`, `경제`, `환율`, `코인`
- **notion**: `notion`, `노션`, `메모`, `문서`
- **todo**: `todo`, `할일`, `태스크`
- **weather**: `weather`, `날씨`, `기상`
- **mp3-player**: `mp3-player`, `로컬음악`, `mp3`
- **wallpaper**: `wallpaper`, `배경화면`, `월페이퍼`, `배경`
- **youtube-music**: `youtube-music`, `유튜브`, `뮤직`, `음악`, `노래`, `youtube`
- **news**: `news`, `뉴스`, `기사`, `소식`
- 예: `@날씨 오늘 우산 챙길까?`, `@스케줄러 내가 오늘 해야 할 일은?`

**3. ⚡ 플러그인 직접 제어 (`/`)**
- `/명령어` 형태는 플러그인이 직접 기능을 처리하는 명시적 커맨드입니다.
- 예: 현재 시스템 창고 상태/도움말 보기(`/help`), 노션 퀵 캡처(`/n`), 일정/할 일 추가(`/todo`)

**4. 🤖 자율 AI 대화 (접두사 없음)**
- 접두사가 없으면 현재 선택된 탭의 AI 모델(Gemini, Ollama 등)에게 일반적인 프롬프트를 전송하여 자율 판단을 맡깁니다.

**💡 부가 옵션: 음소거 (Silent Mode)**
- 명령어 끝에 `--m` 또는 `--mute` 파라미터를 붙이면 해당 응답에 한해 아바타가 말하지 않고 텍스트 형태로만 조용하게 브리핑합니다.

![터미널 HUD 스크린샷 2 (이 부분에 두 번째 스크린샷이 들어갈 예정입니다)](img/user_guide_terminal2.png)
*(인텐트 라우팅을 활용한 스마트 브리핑 화면)*
...
> |---|---|---|
> | 👤 **Users** | 📖 [USER GUIDE](docs/manual/USER_GUIDE.md) | Setup & configuration guide |
> | 🔧 **Developers** | 🚀 [QUICK START](docs/for_developer/QUICK_START_en.md) | First plugin in 5 minutes |
> | 🔧 **Developers** | 📐 [PLUGIN SPEC](docs/for_developer/PLUGIN_SPEC_en.md) | Complete rules, API & schema reference |
> | 🏗️ **Architecture** | 🏗️ [ARCHITECTURE](docs/ARCHITECTURE_en.md) | System design, data flow & patterns |
> | 🤖 **AI Agents** | 🤖 [AI PROMPT (EN)](docs/for_developer/AI_AGENT_PROMPT_en.md) | Copy-paste to ChatGPT/Claude for instant widget dev |

## 🤖 AI-Powered Plugin Development

AEGIS is designed so that **AI agents can directly develop plugins**.

```bash
# Step 1: Auto-generate scaffolding (~20 rules auto-enforced)
python create_plugin.py --id my-widget --name "My Widget"

# Step 2: Copy AI_AGENT_PROMPT.md to ChatGPT/Claude
# "Build me a real-time Bitcoin price widget" → Instant development
```

**Why is this possible?**
- 📋 **Structured docs**: All rules/API/schemas defined in a single [PLUGIN_SPEC.md](docs/for_developer/PLUGIN_SPEC.md)
- 🤖 **AI-ready prompt**: Copy [AI_AGENT_PROMPT.md](docs/for_developer/AI_AGENT_PROMPT.md) to any AI for instant plugin dev
- 🛠️ **Boilerplate generator**: `create_plugin.py` auto-applies ~20 architecture rules
- 🔌 **Plug & Play**: Add/remove folders to extend/remove features — no core file changes
...
- **Global I18n Support ✨**: Automatic language switching (Korean/English) and localized AI personas based on user settings.
- **Unified Messaging (Discord Bot) ✨**: Interact with AEGIS, check schedules, and manage alarms from outside the dashboard via Discord.
...
### 🖥️ AEGIS Unified Command Routing (v3.4.0)
Starting from v3.4.0, all input channels (Web Terminal, Discord) share the same command routing logic. The prefix determines the AI's operating mode.

**1. 🧠 Hybrid Reasoning ( `/@` )**
- Combines **Local Widget Context** with **Real-time Web Search** for the most accurate and intelligent reasoning.
- e.g., `/@ Recommend lunch menu based on my calendar today`

**2. 🏠 Local-Only Summary ( `/` )**
- Answers using **only your local system data** without calling external search. Ideal for secure information or status reports. (Includes legacy alias commands)
- e.g., `/news`, `/calendar`, `/briefing on stocks`

**3. 🌐 Deterministic Web Search ( `/#` )**
- Bypasses local context and uses **external web search** 100% of the time for the latest public information.
- e.g., `/# Tell me NVIDIA's current stock price`

**4. 🤖 Autonomous Conversation (No prefix)**
- The AI autonomously decides whether to use context or search for a natural conversation flow.

**Currently Recognized Aliases:**
- **scheduler**: `scheduler`, `routine`, `reservation`
- **calendar**: `calendar`, `schedule`
- **stock**: `stock`, `market`
- **emails**: `emails`, `mail`, `gmail`
- **system-stats**: `system-stats`, `system`, `status`, `computer`
- **finance**: `finance`, `economy`, `exchange`, `coin`
- **notion**: `notion`, `memo`, `document`
- **todo**: `todo`, `task`
- **weather**: `weather`, `climate`
- **mp3-player**: `mp3-player`, `localmusic`, `mp3`
- **wallpaper**: `wallpaper`, `background`
- **youtube-music**: `youtube-music`, `youtube`, `music`, `song`
- **news**: `news`, `article`

**💡 Additional Option: Silent Mode**
- Append `--m` or `--mute` to the end of any command for text-only briefing without TTS voice output.

![Terminal HUD Screenshot 2 (Image placeholder for the second screenshot)](img/insert_terminal_hud_image2_en.png)
*(Smart briefing screen utilizing Intent Routing)*
