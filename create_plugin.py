#!/usr/bin/env python3
"""
AEGIS Plugin-X 보일러플레이트 생성기 (v1.5.0) [v4.0 하이브리드 호환]
==============================================
...
def create_manifest(
    ...
    # [v4.0] Exhaustive Meta Fields
    manifest["engine_version"] = 4
    manifest["priority"] = 100
    manifest["complexity"] = 1
    manifest["hybrid_level"] = 2  # Standard Iframe isolation
    manifest["icon"] = "🔌"

    if csp_domains:
        manifest["csp_domains"] = {"connect-src": csp_domains}

    # [v4.0] Exhaustive Exports (Sensors, Commands, Actions)
        ...
        "actions": [
            {
                "id": "sample_action",
                "name": f"{name} 샘플 액션",
                "description": f"{name}의 기능을 실행하는 샘플 액션입니다.",
                "type": "terminal_command",
                "payload": {
                    "command": f"/{plugin_id} status"
                }
            }
        ]
    }
...
def create_widget_html(plugin_id: str, name: str, lang: str = "ko") -> str:
  ...
  ⛔ Rules:
    1. This file is injected into Iframe via innerHTML.
    2. <script> tags will not execute. Put all logic in widget.js.
    3. DOM access: root.querySelector() only.
-->
<div class="{plugin_id}-container">
    ...
    <div class="{plugin_id}-actions no-drag">
        <button class="action-btn no-drag" data-action="refresh">{t["refresh"]}</button>
    </div>
</div>
'''
 ...
 * ==========================================
 * ⛔ 필수 규칙:
 *   1. 모든 DOM 접근은 root.querySelector()를 사용
...
export default {{
    ...
    /**
     * 플러그인 초기화 (시스템이 1회 호출)
     * @param {{HTMLElement}} root - 위젯의 최상위 루트 (document)
    ...
    init: async function(root, context) {{
        ...
        // [v4.0] Sub-module loading example (Uncomment to use)
        // const MyModule = await import(context.resolve('assets/my_module.js'));
        // this.myModule = new MyModule.default(context);

        // ═══════════════════════════════════════════════════
        // [1] 이벤트 위임 (Event Delegation) ⛔ stopPropagation 필수
        // ═══════════════════════════════════════════════════
        this.root.addEventListener('click', (e) => {{
            const btn = e.target.closest('[data-action]');
            if (!btn) return;
            e.stopPropagation(); // ⛔ 드래그 동작 간섭 방지
        ...
        // ═══════════════════════════════════════════════════
        context.registerCommand('/' + context.id, (param) => {{
    ...
    async refresh() {{
        try {{
            ...
            const statusEl = this.root.querySelector('#status-text');
            if (statusEl) {{
